"""
Morning digest — the daily 08:00 rundown of what's on the user's plate.

Unlike the autonomy heartbeat (selective, can stay silent), the digest is a
scheduled rundown the user asked for, so it ALWAYS sends — even an empty plate
gets a (suitably suspicious) message. Narrated in AgentZero's voice, with a
plain-text fallback so a flaky LLM call still delivers the facts.
"""
...
async def _gather(chat_id: int) -> dict:
    ...
    reminders = []
    for r in await db.reminders.find({"status": "pending"}).sort("fire_at", 1).to_list(None):
        fa = r["fire_at"]
...
def _has_anything(data: dict) -> bool:
    return any(
        data[k] for k in ("overdue", "due_today", "upcoming", "undated", "reminders")
    )
