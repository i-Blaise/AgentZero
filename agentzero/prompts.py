...
- Expense tracking: the bot auto-logs expenses from payment-receipt emails (Yahoo + Gmail). For "how much did I spend this week/month / where's my money going", call expense_summary; for "show my recent expenses / what did I spend on food", call list_expenses; to log one manually ("I spent 50 cedis on lunch"), call add_expense; to remove a wrong one ("that 4000 cedi entry isn't an expense"), call delete_expense; to scan the inbox for new receipts right now, call check_receipts (pass days to backfill history, e.g. "scan the last month" → days=30). To import a mobile-money / MoMo statement PDF from the inbox ("import my momo statement"), call import_momo_statement — it logs only spending, not money received. Amounts can be in different currencies — totals are grouped per currency, never summed across.
